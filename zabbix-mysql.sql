...
/*
   存储过程的使用：
   存储过程如果单独使用：CALL partition_maintenance(zabbix, ‘history_uint’, 31, 24, 10);
   解释：history_uint表最多保存31天的数据，每隔24小时生成一个分区，这次一共生成10个分区
   可以使用一个存储过程来实现7张表的分区：
*/
DELIMITER $$
...
-- 添加定时任务，使工作自动化：
--01 01 * * * mysql -uzabbix -pzabbix zabbix -e "CALL partition_maintenance_all('zabbix');"
